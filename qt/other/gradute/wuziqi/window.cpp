...
void Window::dealMenu(void)
{
    ...
    QMenu * AlgorithmMenu = new QMenu(tr("难度"), MenuBar);
	QMenu * HelpMenu = new QMenu(tr("帮助"), MenuBar);
	//添加游戏菜单项
    QAction * StartGame = new QAction(tr("开始"), GameMenu);
    QAction * StopGame = new QAction(tr("结束"), GameMenu);
    QAction * QuitGame = new QAction(tr("退出"), GameMenu);
	...
	//添加算法选择菜单
    QAction * Algorithm1 = new QAction(tr("简单"), AlgorithmMenu);
    QAction * Algorithm2 = new QAction(tr("一般"), AlgorithmMenu);
    QAction * Algorithm3 = new QAction(tr("困难"), AlgorithmMenu);

    QAction * Against = new QAction(tr("机器对战"), AlgorithmMenu);
...
{
    QMessageBox::information(this, tr("关于游戏"),tr("五子棋人际对战，该软件受GPL协议保护，侵权必究"));
}
