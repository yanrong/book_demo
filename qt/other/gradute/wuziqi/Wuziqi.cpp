...
bool Wuziqi::ChangePos(const POINT & srcPos, POINT & desPos,const int & beginPos, const int & endPos)//将座标进行转换
{
...
bool Wuziqi::Fall(const POINT & ps, const int & who,const int & beginPos, const int & endPos)
{
    POINT tempPos;
...
//评分函数
void Wuziqi::SetScore(const int i,const int j,const int Who,vector<vector<int> > &State)
{
