...
bool
isPrime(unsigned int m, unsigned int n)
{
    unsigned int i = 0;
    bool Flag = true;
...
unsigned int
MakePrivatedKeyd(unsigned int uiP, unsigned int uiQ)
{
    unsigned int i = 0;
...
unsigned int
MakePairkey(unsigned int uiP, unsigned int uiQ, unsigned int uiD)
{
    bool bFlag = true;
    unsigned int i = 0, e;
    unsigned int z = (uiP - 1) * (uiQ - 1);
...
unsigned int
GetPairKey(unsigned int &d, unsigned int &e)
{
    d = pairkey.d;
    e = pairkey.e;
    return pairkey.n;
...
unsigned int
GetPrivateKeyd(unsigned int iWhich)
{
    if (pset.size >= iWhich)
...
void
rsa_decrypt(int n, int d, int *&cw, int cLength, char *mw)
{
    int i = 0, j = -1;
    int64 temInt = 0;
